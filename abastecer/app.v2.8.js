(() => {
  ...
  async function salvarRascunho(estrito=false){if(state.selfie||state.rodadaId)try{const d=dadosRascunho();await dbPut('drafts',d);if(d.operador)await dbPut('drafts',{...d,key:'operador:'+normalizar(d.operador)});}catch(e){if(estrito)throw new Error('Não foi possível guardar. Mantenha a tela aberta e tente de novo.');mensagem('Não foi possível guardar o rascunho neste navegador. Mantenha a página aberta até confirmar.');}}
  ...
  async function limparOperador(){
    ...
    fecharCamera();await salvarRascunho(true);await dbDelete('drafts','current');
  ...
  async function pausarETrocar(){if(state.busy){mensagem('Aguarde terminar o processamento.');return;}try{await limparOperador();mensagem('Rascunho guardado. Selecione o nome de quem vai continuar.');}catch(e){mensagem(e.message);}}
  ...
  function renderEquipamentos(){
    const lista=(state.config.catalogo||{})[state.local]||{},detalhes=(state.progresso&&state.progresso.detalhes)||[];
    $('equipmentList').innerHTML=Object.keys(lista).sort().map(nome=>{
      const d=detalhes.find(x=>normalizar(x.equipamento)===normalizar(nome));
      const info=d&&d.capturadaEm?new Date(d.capturadaEm).toLocaleString('pt-BR')+' · '+d.operador+(d.horarioEstimado?' · horário estimado':''):'Ainda sem contagem';
      return `<button type="button" class="equipment-button" data-equipment="${escapeHtml(nome)}"><strong>${escapeHtml(nome)}</strong><small>${escapeHtml(info)}</small><span>Fotografar</span></button>`;
    }).join('');
  }
  ...
  function renderFotos(){document.querySelectorAll('#photoGrid .photo').forEach(x=>x.remove());state.fotos.forEach((foto,i)=>{const d=document.createElement('div');d.className='photo';d.innerHTML=`<img alt="Foto ${i+1}" src="${foto}"><button type="button" data-index="${i}" aria-label="Excluir foto">×</button><span>Foto ${i+1}</span>`;$('photoGrid').appendChild(d);});$('addPhoto').disabled=state.fotos.length>=6;$('photoLimit').textContent=state.fotos.length+'/6 fotos';$('analyzeButton').disabled=state.fotos.length===0;}

  async function processarImagem(file,tipo){if(!file||!file.type.startsWith('image/'))throw new Error('Selecione uma imagem válida.');const bitmap=await carregarBitmap(file);let w=bitmap.width,h=bitmap.height;const escala=Math.min(1,(tipo==='selfie'?1280:2048)/Math.max(w,h));w=Math.max(1,Math.round(w*escala));h=Math.max(1,Math.round(h*escala));let qualidade=tipo==='selfie'?.66:.82,data;for(let passo=0;passo<4;passo++){const canvas=document.createElement('canvas');canvas.width=w;canvas.height=h;const ctx=canvas.getContext('2d',{alpha:false});ctx.drawImage(bitmap,0,0,w,h);data=canvas.toDataURL('image/jpeg',qualidade);if(data.length<=(tipo==='selfie'?650000:750000))break;qualidade-=.1;w=Math.round(w*.88);h=Math.round(h*.88);}if(bitmap.close)bitmap.close();return data;}
  ...
  async function analisar(){if(state.busy||!state.fotos.length)return;state.busy=true;overlay('Contando com IA','Identificando os produtos desta geladeira.');try{const r=await retry(()=>server('analisarImagens',{sessionToken:state.sessionToken,auditoriaId:state.auditoriaId,rodadaId:state.rodadaId,local:state.local,geladeira:state.geladeira,fotos:state.fotos,capturadaEm:Math.min(...state.fotoHorarios)}),1);state.auditoriaId=r.auditoriaId;state.desvio=r.desvio;state.meta=r.meta;state.comprovanteAnalise=r.comprovanteAnalise;state.itens=r.itens.map(x=>({...x,conferido:false,contado_humano:x.planejado&&Number(x.contado_ia)===0?'':x.contado_ia}));state.mostrarTodos=false;renderReview();await salvarRascunho();show('reviewScreen');status('Conferência');}catch(e){mensagem((navigator.onLine?'Falha na análise: ':'Sem conexão: ')+e.message);}finally{state.busy=false;hideOverlay();}}
  function itensVisiveis(){return state.itens.reduce((s,x)=>s+Number(x.contado_humano||0),0);}
  function precisaConferir(x){return (x.planejado||Number(x.contado_ia)>0||Number(x.contado_humano)>0)&&x.conferido!==true;}
  function renderReview(){
    const pendentes=state.itens.filter(precisaConferir).length;
    $('replenishmentSummary').textContent=pendentes?pendentes+' produtos para conferir. A IA mostra apenas o que conseguiu ver.':itensVisiveis()+' unidades conferidas por você';
    $('deviationCard').classList.toggle('hidden',!state.desvio);$('deviationText').textContent=state.desvio||'';
    const visiveis=state.itens.map((x,i)=>({x,i})).filter(({x})=>state.mostrarTodos||x.planejado||Number(x.contado_ia)>0||Number(x.contado_humano)>0);
    $('showAllSkus').classList.toggle('hidden',state.mostrarTodos||visiveis.length===state.itens.length);
    $('itemsList').innerHTML=visiveis.map(({x,i})=>{
      const fora=!x.planejado&&Number(x.contado_humano)>0,pendente=precisaConferir(x);
      const leitura=Number(x.contado_ia)===0&&x.confianca!=='alta'?'Não identificado na foto':x.contado_ia+' visíveis na foto';
      return `<div class="item-card ${pendente?'needs-review':''}"><div><h3>${escapeHtml(x.produto)}</h3><div class="item-meta"><span>${escapeHtml(leitura)}</span></div><p class="count-evidence">${escapeHtml(x.evidencia||'Confira também o fundo e as unidades empilhadas.')}</p><span class="repor ${pendente?'off-plan':'complete'}">${pendente?'CONFERIR TOTAL':'CONFERIDO POR VOCÊ'}</span>${fora?'<small>Encontrado fora da posição prevista</small>':''}</div><div class="count-actions"><div class="counter"><button data-delta="-1" data-index="${i}" aria-label="Diminuir ${escapeHtml(x.produto)}">−</button><input data-index="${i}" inputmode="numeric" type="number" min="0" placeholder="?" value="${escapeHtml(x.contado_humano)}" aria-label="Total de ${escapeHtml(x.produto)}"><button data-delta="1" data-index="${i}" aria-label="Aumentar ${escapeHtml(x.produto)}">+</button></div>${pendente?`<button class="secondary verify-count" data-confirm="${i}">Conferi o total</button>`:''}</div></div>`;
    }).join('');
  }

  function ajustar(i,delta){state.itens[i].conferido=true;state.itens[i].contado_humano=Math.max(0,Math.min(1000000,Math.floor(Number(state.itens[i].contado_humano)||0)+delta));renderReview();salvarRascunho();}

  function payloadSalvamento(){return {sessionToken:state.sessionToken,rodadaId:state.rodadaId,auditoriaId:state.auditoriaId,comprovanteAnalise:state.comprovanteAnalise,contagensFinais:state.itens.map(x=>({skuId:x.skuId,contado_humano:x.contado_humano,conferido:x.conferido===true}))};}
  ...
  async function salvar(){
    ...
    const pendente=state.itens.find(precisaConferir);if(pendente){mensagem('Confira o total de '+pendente.produto+' e toque em “Conferi o total”.');return;}
  ...
  function mostrarSucesso(pendente){const p=state.progresso||{concluidas:0,total:0,faltantes:[]};const completa=!pendente&&p.total>0&&!p.faltantes.length;$('successTitle').textContent=pendente?'Contagem guardada':'Geladeira registrada';$('successText').textContent=pendente?'Aguardando envio. Retome com seu nome quando a conexão voltar.':'A quantidade entrou no total deste local.';$('successSummary').textContent=p.concluidas+' de '+p.total+' equipamentos com contagem';$('auditIdLabel').textContent=pendente?'Ainda não entrou no estoque confirmado.':completa?'Confira os horários antes de gerar o pedido.':'Falta contar: '+p.faltantes.join(', ');$('nextAudit').textContent=pendente?'Voltar ao local':completa?'Conferir local e pedido':'Continuar contando';$('successMore').classList.toggle('hidden',!completa);show('successScreen');status(pendente?'Pendente de envio':'Contagem do local');salvarRascunho();}
  ...
  $('checkinButton').addEventListener('click',checkin);$('changeOperator').addEventListener('click',pausarETrocar);
  ...
  $('addPhoto').addEventListener('click',()=>$('fridgeInput').click());$('fridgeInput').addEventListener('change',async e=>{if(!e.target.files.length||state.fotos.length>=6)return;try{overlay('Preparando foto','Otimizando a imagem para envio.');const capturadaEm=Date.now()+state.clockOffset;state.fotos.push(await processarImagem(e.target.files[0],'geladeira'));state.fotoHorarios.push(capturadaEm);renderFotos();salvarRascunho();}catch(x){mensagem(x.message);}finally{e.target.value='';hideOverlay();}});
  ...
  $('itemsList').addEventListener('click',e=>{const confirmar=e.target.closest('button[data-confirm]');if(confirmar){const x=state.itens[Number(confirmar.dataset.confirm)];if(x.contado_humano===''||!Number.isSafeInteger(Number(x.contado_humano))||Number(x.contado_humano)<0){mensagem('Digite o total físico. Use 0 somente se não houver nenhuma unidade.');return;}x.conferido=true;renderReview();salvarRascunho();return;}const b=e.target.closest('button[data-delta]');if(b)ajustar(Number(b.dataset.index),Number(b.dataset.delta));});
  $('itemsList').addEventListener('input',e=>{if(e.target.matches('input[data-index]')){const i=Number(e.target.dataset.index);state.itens[i].contado_humano=e.target.value;state.itens[i].conferido=false;const valido=e.target.value!==''&&Number.isSafeInteger(Number(e.target.value))&&Number(e.target.value)>=0&&Number(e.target.value)<=1000000;e.target.setCustomValidity(valido?'':'Informe uma quantidade inteira válida.');$('replenishmentSummary').textContent='Confira o total físico antes de confirmar a geladeira.';salvarRascunho();}});
  $('itemsList').addEventListener('change',e=>{if(e.target.matches('input[data-index]'))renderReview();});
  $('showAllSkus').addEventListener('click',()=>{state.mostrarTodos=true;renderReview();});$('redoButton').addEventListener('click',()=>{show('captureScreen');status('Fotografando');});$('saveButton').addEventListener('click',salvar);$('nextAudit').addEventListener('click',proxima);$('successMore').addEventListener('click',proxima);$('resumeRound').addEventListener('click',sincronizarPendentes);$('finishLocal').addEventListener('click',finalizarLocalPeloOperador);
  ...
  $('pauseCapture').addEventListener('click',pausarETrocar);$('pauseReview').addEventListener('click',pausarETrocar);
