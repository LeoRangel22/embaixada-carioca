(() => {
  ...
  const state = {
    ...
    fotos:[], local:'', geladeira:'', auditoriaId:'', rodadaId:'', rodadaLocal:'',
  ...
  function dadosRascunho(){return {key:'current',updatedAt:Date.now(),operador:state.operador||nomeSelecionado(),selfie:state.selfie,fotos:state.fotos,local:state.local,geladeira:state.geladeira,auditoriaId:state.auditoriaId,rodadaId:state.rodadaId,rodadaLocal:state.rodadaLocal,desvio:state.desvio,itens:state.itens,meta:state.meta,comprovanteAnalise:state.comprovanteAnalise,progresso:state.progresso,solicitacao:state.solicitacao};}
  async function salvarRascunho(){if(state.selfie||state.rodadaId)try{await dbPut('drafts',dadosRascunho());}catch(e){mensagem('Não foi possível guardar o rascunho neste navegador. Mantenha a página aberta até confirmar.');}}
  async function apagarRascunho(){await dbDelete('drafts','current');}
  ...
  async function boot(){
    ...
    try{
      ...
      if(token){
        try{
          const resposta=await server('carregarCatalogo',token);state.sessionToken=token;state.operador=resposta.operador;state.config.catalogo=resposta.catalogo;state.config.metasLocais=resposta.metasLocais;preencherLocais();
  ...
  async function checkin(){
    if($('checkinButton').disabled||state.busy)return;if(state.rodadaId&&!state.solicitacao&&state.operador!==nomeSelecionado()){mensagem('Há uma rodada em andamento de '+state.operador+'. Selecione esse nome para continuar.');return;}state.busy=true;overlay('Iniciando turno','Enviando a selfie uma única vez.');
    try{const r=await server('abrirSessao',{operador:nomeSelecionado(),pin:$('accessPin').value,selfie:state.selfie});state.sessionToken=r.sessionToken;state.operador=r.operador;state.checkinId=r.checkinId;sessionStorage.setItem('reposicaoSessionToken',state.sessionToken);$('accessPin').value='';const c=await server('carregarCatalogo',state.sessionToken);state.config.catalogo=c.catalogo;state.config.metasLocais=c.metasLocais;preencherLocais();mostrarOperador();const recuperado=await restaurarRascunho(true);if(!recuperado){show('selectScreen');status('Seleção');}sincronizarPendentes();}catch(e){mensagem(e.message);}finally{state.busy=false;hideOverlay();if(state.sessionToken)sincronizarPendentes();}
  }
  async function limparOperador(){
    if(state.busy)return;
    const fila=(await dbAll('outbox')||[]).filter(x=>x.operador===state.operador);
    if(fila.length || (state.rodadaId&&!state.solicitacao)){
      mensagem('Conclua ou sincronize a rodada atual antes de trocar o auditor.');return;
    }
    fecharCamera();await apagarRascunho();
    Object.assign(state,{sessionToken:'',operador:'',checkinId:'',selfie:null,fotos:[],local:'',geladeira:'',auditoriaId:'',rodadaId:'',rodadaLocal:'',desvio:null,itens:[],meta:null,comprovanteAnalise:'',progresso:null,solicitacao:null,pedidoBlob:null});
  ...
  function iniciarCaptura(){if(!state.rodadaId){state.rodadaId=uuid();state.rodadaLocal=state.local;state.progresso=progressoInicial(state.local);}state.fotos=[];state.itens=[];state.desvio=null;state.meta=null;state.comprovanteAnalise='';state.auditoriaId=uuid();state.mostrarTodos=false;$('captureTitle').textContent=state.geladeira;renderFotos();salvarRascunho();show('captureScreen');status('Fotografando');$('fridgeInput').click();}
  ...
  async function salvar(){
    ...
    try{
      ...
      if(r.prontoParaFinalizar){
        try{await concluirPedido();}catch(e){await proxima();mensagem('Contagens salvas. Toque em “Concluir e gerar pedido” para tentar novamente. '+e.message);}
      }else mostrarSucesso(false);
  ...
  function prepararDepoisDaGeladeira(){state.fotos=[];state.itens=[];state.desvio=null;state.meta=null;state.comprovanteAnalise='';state.auditoriaId='';state.geladeira='';state.mostrarTodos=false;salvarRascunho();}
  ...
  async function sincronizarPendentes(){
    ...
    try{
      ...
      if(state.rodadaId){
        const r=await server('consultarRodada',{sessionToken:state.sessionToken,rodadaId:state.rodadaId});
        if(r.encontrada){
          state.progresso=r.progresso;
          if(r.solicitacao){state.solicitacao=r.solicitacao;renderPedido();show('requestScreen');status('Pedido pronto');}
          else if(r.progresso.total>0&&!r.progresso.faltantes.length){await concluirPedido();}
  ...
  $('locationSelect').addEventListener('change',()=>carregarGeladeiras(false));$('fridgeSelect').addEventListener('change',renderPlanograma);$('startCapture').addEventListener('click',iniciarCaptura);
  $('addPhoto').addEventListener('click',()=>$('fridgeInput').click());$('fridgeInput').addEventListener('change',async e=>{if(!e.target.files.length||state.fotos.length>=4)return;try{overlay('Preparando foto','Otimizando a imagem para envio.');state.fotos.push(await processarImagem(e.target.files[0],'geladeira'));renderFotos();salvarRascunho();}catch(x){mensagem(x.message);}finally{e.target.value='';hideOverlay();}});
  $('photoGrid').addEventListener('click',e=>{const b=e.target.closest('.photo button');if(b){state.fotos.splice(Number(b.dataset.index),1);renderFotos();salvarRascunho();}});
  ...
  $('showAllSkus').addEventListener('click',()=>{state.mostrarTodos=true;renderReview();});$('redoButton').addEventListener('click',()=>{show('captureScreen');status('Fotografando');});$('saveButton').addEventListener('click',salvar);$('nextAudit').addEventListener('click',proxima);$('resumeRound').addEventListener('click',sincronizarPendentes);
