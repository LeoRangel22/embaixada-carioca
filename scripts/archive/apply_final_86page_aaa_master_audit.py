...
UX/conversão, design/marca, contraste, imagens/performance básica e integridade técnica.

V2: auditoria recalibrada após validação visual real.
- Aceita o Visual Readability Reality Fix como trava válida de contraste.
- Não trata números visíveis como `7.779 avaliações` como erro técnico.
- Não trata chaves de CSS/JS/JSON-LD como template quebrado.
- Avalia páginas utilitárias separadamente, sem exigir nav/brand locks comerciais.
"""
...
GEO_MARKERS = (
    "application/ld+json", "FAQPage", "Restaurant", "LocalBusiness",
    "Direct answer", "Resposta direta", "Respuesta directa",
    "ec-final-geo-answer", "ec-sprint2-geo",
)
...
def code_free_text(raw: str) -> str:
    """Texto visível para detectar tokens realmente quebrados, sem CSS/JS/JSON."""
...
def has_final_design_lock(text: str) -> bool:
...
def has_readability_lock(text: str) -> bool:
    return any(marker in text for marker in [
...
def has_webkit_reset(text: str) -> bool:
    return "-webkit-text-fill-color:currentcolor!important" in text.replace(" ", "").lower()


def audit_language(rel: str, text: str, kind: str) -> tuple[float, dict[str, bool]]:
    ...
    checks = {
        ...
        "has_visible_copy": len(body_text) > (100 if kind != "utility" else 20),
    ...
    if lang == "pt":
        ...
        checks["en_has_english_markers"] = any(x in body_text for x in ["rio de janeiro", "sugarloaf", "restaurant", "breakfast", "lunch", "quote"]) or kind == "utility"
        checks["en_no_portuguese_cta_leak"] = "solicitar cotação" not in body_text and "reservar via tagme" not in body_text
    else:
        checks["es_has_spanish_markers"] = any(x in body_text for x in ["río", "azúcar", "restaurante", "desayuno", "almuerzo", "cotización"]) or kind == "utility"
...
def audit_seo(rel: str, text: str, kind: str) -> tuple[float, dict[str, bool]]:
    ...
    checks = {
        ...
        "indexable_structure": "noindex" not in text.lower() or kind == "utility",
    }
    return score_from_checks(checks), checks
...
def audit_geo(rel: str, text: str, kind: str) -> tuple[float, dict[str, bool]]:
    ...
    text_low = text.lower()
    checks = {
        "jsonld_present": "application/ld+json" in text_low or kind == "utility",
        ...
        "entity_terms_present": any(term in text_low for term in ["morro da urca", "pão de açúcar", "pao de acucar", "sugarloaf", "pan de azúcar", "bondinho"]) or kind == "utility",
        "hasmap_or_address_or_geo": any(term in text_low for term in ["hasmap", "av. pasteur", "avenida pasteur", "22°", "latitude", "longitude", "morro da urca"]) or kind == "utility",
    }
    return score_from_checks(checks), checks
...
def audit_ux(rel: str, text: str, kind: str) -> tuple[float, dict[str, bool]]:
    ...
    checks = {
        "top_nav_present": ("<nav" in text_low and "class=\"top" in text_low) or kind == "utility",
        "reservation_cta_present": any("tagme" in a or "reserv" in a for a in anchors) or kind == "utility",
        "menu_or_content_cta_present": any(x in text_low for x in ["cardápio", "cardapio", "menu", "como chegar", "how to get", "cómo llegar"]) or kind == "utility",
...
def audit_design(rel: str, text: str, kind: str) -> tuple[float, dict[str, bool]]:
    text_low = text.lower()
    checks = {
        "brand_manual_lock": "ec-brand-manual-alignment" in text or kind == "utility",
        "final_design_lock": has_final_design_lock(text) or kind == "utility",
        "brand_palette_present": all(x in text_low for x in ["#00405a", "#f59b1e", "#ede2c9"]) or kind == "utility",
        "typography_present": "catamaran" in text_low and "verdana" in text_low or kind == "utility",
        "logo_present_or_utility": "logo" in text_low or kind == "utility",
        "button_hierarchy_lock": ("Somente reserva / TagMe fica laranja" in text or "apenas reserva/tagme" in text_low or "ec-visual-readability-reality-fix" in text or kind == "utility"),
...
def audit_contrast(rel: str, text: str, kind: str) -> tuple[float, dict[str, bool]]:
    ...
    checks = {
        "legibility_lock_present": has_readability_lock(text) or kind == "utility",
        "readability_emergency_lock": ("ec-aaa-readability-emergency-fix" in text or "ec-visual-readability-reality-fix" in text) or kind == "utility",
        "hard_readability_lock": ("ec-lunch-photos-global-readability-hardfix" in text or "ec-visual-readability-reality-fix" in text) or kind == "utility",
        "webkit_reset": has_webkit_reset(text) or kind == "utility",
        "dark_background_light_text": ("rgba(246,239,222,.90)!important" in compact or "rgba(246,239,222,.88)!important" in compact or "rgba(237,226,201,.88)!important" in compact) or kind == "utility",
        "light_cards_dark_text": ("--ec-vr-gray:#485156" in compact or "--ec-gray:#485156" in compact) and ("--ec-vr-blue:#00405a" in compact or "--ec-blue:#00405a" in compact) or kind == "utility",
    }
    return score_from_checks(checks), checks
...
def audit_images_perf(rel: str, text: str, kind: str) -> tuple[float, dict[str, bool]]:
    ...
    img_alt_ok = True if not imgs else all(ALT_RE.search(img) for img in imgs)
    checks = {
        ...
        "cache_or_perf_report_lock": "ec-lunch-photos-global-readability-hardfix" in text or "ec-brand-manual-alignment" in text or "ec-visual-readability-reality-fix" in text or kind == "utility",
...
def audit_integrity(rel: str, text: str, kind: str) -> tuple[float, dict[str, bool]]:
    visible = code_free_text(text)
    checks = {
        "no_old_event_email": not WRONG_EMAIL_RE.search(text),
        "no_broken_template_tokens": not any(tok in visible for tok in ["{{", "}}", "[object Object]", "undefined", "Lorem ipsum"]),
...
def write_reports(rows: list[dict[str, object]]) -> None:
    ...
    category_names = ["language", "seo", "geo_aio_sai", "ux_conversion", "design_brand", "contrast_readability", "images_performance", "technical_integrity"]
    cat_avgs = {cat: round(sum(float(r[f"score_{cat}"]) for r in rows) / total, 1) if rows else 0 for cat in category_names}
    ...
    lines.extend([
        ...
        "- Visual Readability Reality Fix é aceito como trava final de contraste real.",
        "- A validação final ainda deve incluir inspeção visual no navegador com cache limpo.",
        "",
