#!/usr/bin/env python3
"""Visual Readability Reality Fix V2 — Embaixada Carioca.

Versão robusta para substituir a primeira tentativa que falhou ao gerar CSV por
misturar linhas de ação com linhas de auditoria. Esta versão mantém o mesmo
objetivo visual, mas grava o CSV com colunas homogêneas.
"""
...
INVALID_PATTERNS = [
    ...
    ("rgba(237.779,201,0.10)", "rgba(237,226,201,0.10)"),
    ("rgba(237.779,201,0.18)", "rgba(237,226,201,0.18)"),
    ("rgba(237.779,201,0.78)", "rgba(237,226,201,0.78)"),
    ("rgba(237.779,201,0.6)", "rgba(237,226,201,0.6)"),
    ("rgba(237.779,201,0.7)", "rgba(237,226,201,0.7)"),
]

FIX_BLOCK = START + r'''
<style id="ec-visual-readability-reality-fix">
/* VISUAL REALITY FIX V2 — correção final de contraste observada no navegador.
   Card claro = texto escuro. Cardápio = nome do prato em verde escuro. */
...
html body main :is(.menu-section,.menu-grid) .menu-item,html body main .menu-item{background:var(--ec-vr-sand)!important;color:var(--ec-vr-gray)!important;border-color:rgba(0,64,90,.16)!important;}
html body main :is(.menu-section,.menu-grid) .menu-item :is(h1,h2,h3,h4,.menu-item-name,.item-name,.dish-name,.title,.card-title),html body main .menu-item :is(h1,h2,h3,h4,.menu-item-name,.item-name,.dish-name,.title,.card-title){color:var(--ec-vr-green)!important;-webkit-text-fill-color:var(--ec-vr-green)!important;font-weight:900!important;text-shadow:none!important;opacity:1!important;}
html body main :is(.menu-section,.menu-grid) .menu-item :is(p,li,span,small,.menu-item-desc,.item-desc,.description,.copy),html body main .menu-item :is(p,li,span,small,.menu-item-desc,.item-desc,.description,.copy){color:var(--ec-vr-gray)!important;-webkit-text-fill-color:var(--ec-vr-gray)!important;text-shadow:none!important;opacity:1!important;}
html body main :is(.menu-section,.menu-grid) .menu-item :is(.price,.menu-item-price,.item-price),html body main .menu-item :is(.price,.menu-item-price,.item-price){color:var(--ec-vr-price)!important;-webkit-text-fill-color:var(--ec-vr-price)!important;font-weight:900!important;text-shadow:none!important;}
...
def normalize_invalid_rgba(text: str) -> tuple[str, int]:
    total = 0
    for bad, good in INVALID_PATTERNS:
    ...
    if text2 != text:
        total += len(re.findall(r"rgba\(237\.779,201,", text))
        text = text2
    return text, total
...
def process(path: Path) -> int:
    ...
    text, bad_count = normalize_invalid_rgba(original)
    text, injected = inject_block(text)
    if injected:
        COUNTERS["css_js_injected"] += 1
    if bad_count:
        COUNTERS["invalid_rgba_fixed"] += bad_count
    if text != original:
...
def audit(path: Path, invalid_fixed_count: int) -> None:
    rel = path.relative_to(ROOT).as_posix()
    ...
    DETAILS.append({"page": rel, "status": status, "invalid_rgba_fixed_count": invalid_fixed_count, **checks})
...
def write_reports() -> None:
    ...
    fieldnames = ["page","status","invalid_rgba_fixed_count","reality_fix_present","menu_titles_dark_green_rule","menu_js_guard_present","light_card_dark_text_rule","invalid_rgba_clean","menu_page_guard"]
...
def main() -> int:
    ...
    for p in sorted(ROOT.rglob("*.html")):
        if is_html(p):
            ...
            fixed_counts[p.relative_to(ROOT).as_posix()] = process(p)
    for p in sorted(ROOT.rglob("*.html")):
        if is_html(p):
            audit(p, fixed_counts.get(p.relative_to(ROOT).as_posix(), 0))
    write_reports()
    return 0
